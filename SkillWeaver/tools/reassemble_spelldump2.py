# tools/reassemble_spelldump2.py
import re, sys, json

pat = re.compile(r"^SWSPELL2_CHUNK\|(\d+)\/(\d+)\|(.*)$")

def main():
    if len(sys.argv) < 3:
        print("Usage: python reassemble_spelldump2.py <dump.txt> <output.json>")
        return
    ...
    if total is None:
        print("No SWSPELL2_CHUNK lines found.")
        return
    ...
    try:
        ...
        print(f"Spells exported: {len(obj.get('spells', {}))}")
    except Exception as e:
        print(f"Error in JSON reassembly: {e}")
