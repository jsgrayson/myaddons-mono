...
function debug:OnInitialize()
  self.profiles = {}
  self.enabled = false
  self.tooltip = CreateFrame('GameTooltip', 'DeepPocketsTooltip', UIParent, 'GameTooltipTemplate') --[[@as GameTooltip]]
end
...
function debug:OnEnable()
  ...
  if self.enabled then
    print("DeepPockets: debug mode enabled")
  end
end
...
function debug:GetTableLink(value)
  ...
  return format("|cff%s|HDeepPockets%s:%s|h[%s]|h|r", debug.colors[valueType], valueType, name, name)
end
