...
function config:CreateConfig()
  local f = form:Create({
    title = 'DeepPockets Settings',
    layout = const.FORM_LAYOUT.STACKED,
    index = true
  })
 f:AddSection({
   title = 'General',
   description = 'General settings for DeepPockets.',
 })
  f:AddCheckbox({
  ...
  f:AddCheckbox({
    ...
    setValue = function(_, value)
      ...
      local sneakyFrame = _G["DeepPocketsSneakyFrame"] ---@type Frame
      if value then
  ...
  f:AddCheckbox({
    ...
    description = 'Enable DeepPockets for bank. If disabled, the default Blizzard bank UI will be used. Requires a UI reload to take effect.',
    getValue = function(_)
    ...
    setValue = function(_, value)
      ...
      StaticPopupDialogs["BETTERBAGS_RELOAD_UI"] = {
        text = "DeepPockets needs to reload the UI for this change to take effect. Reload now?",
        button1 = "Yes",
        button2 = "No",
        OnAccept = function()
          ReloadUI()
