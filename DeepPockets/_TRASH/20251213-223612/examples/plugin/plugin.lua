--[[
  This is an example of how to create a category plugin for DeepPockets.
  Your addon must be a loadable addon, and it must have a .toc file,
  and it must have a dependency on DeepPockets.
  You do not need to bundle Ace or any other library with your addon, as
  DeepPockets will provide them for you.
]]--

-- This will get a handle to the DeepPockets addon.
---@class DeepPockets: AceAddon
local addon = LibStub('AceAddon-3.0'):GetAddon("DeepPockets")
...
-- the API for creating a configuration entry in the DeepPockets config panel.
