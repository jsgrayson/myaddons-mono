...
function Posting:CreateGroupsUI()
    ...
    -- Warehousing Buttons
    local moveBankBtn = CreateFrame("Button", nil, detailsFrame, "UIPanelButtonTemplate")
    moveBankBtn:SetSize(100, 25)
    moveBankBtn:SetPoint("TOPLEFT", 10, -35)
    moveBankBtn:SetText("To Bank")
    moveBankBtn:SetScript("OnClick", function()
        Posting:MoveGroupToBank()
    end)
    
    local moveBagsBtn = CreateFrame("Button", nil, detailsFrame, "UIPanelButtonTemplate")
    moveBagsBtn:SetSize(100, 25)
    moveBagsBtn:SetPoint("TOPRIGHT", -10, -35)
    moveBagsBtn:SetText("To Bags")
    moveBagsBtn:SetScript("OnClick", function()
        Posting:MoveGroupToBags()
    end)
    
    -- Adjust Add Item button position
    addItemBtn:ClearAllPoints()
    addItemBtn:SetPoint("TOP", 0, -5) -- Move up slightly or keep centered
    -- Actually, let's rearrange:
    -- Add Item (Top Center)
    -- To Bank (Left) | To Bags (Right)
    
    addItemBtn:SetPoint("TOP", 0, -35)
    moveBankBtn:SetPoint("TOPLEFT", 10, -65)
    moveBagsBtn:SetPoint("TOPRIGHT", -10, -65)
    
    -- Adjust list position
    local itemList = GoblinAI:CreateScrollFrame(detailsFrame, 410, 290) -- Reduced height
    itemList:SetPoint("TOP", 0, -95)
...
function Posting:MoveGroupToBank()
    if not self.selectedGroup then return end
    
    -- Check if bank is open
    -- BankFrame is the classic frame, but could be others. 
    -- Reliable check: GetNumBankSlots() > 0? No.
    -- Event based? Or just check if frame is shown.
    local bankOpen = BankFrame and BankFrame:IsShown()
    if not bankOpen then
        print("|cFFFF0000Goblin AI:|r You must be at the bank!")
        return
    end
    ...
    print("|cFFFFD700Goblin AI:|r Moving group '" .. group.name .. "' to Bank...")
    
    -- Iterate bags and move items
    for bag = 0, 4 do
        for slot = 1, C_Container.GetContainerNumSlots(bag) do
            local itemID = C_Container.GetContainerItemID(bag, slot)
            if itemID then
                -- Check if item is in group
                for _, targetID in ipairs(group.items) do
...
function Posting:MoveGroupToBags()
    if not self.selectedGroup then return end
    ...
    print("|cFFFFD700Goblin AI:|r Moving group '" .. group.name .. "' to Bags...")
    
    -- Iterate bank bags and move items
    -- Bank (-1) and Bank Bags (5-11)
    local containers = {-1}
    for i = 5, 11 do table.insert(containers, i) end
    -- Reagent Bank (-3)
