# SKILLWEAVER VISION ENGINE v25.9
...
class VisionSystem:
    ...
    def decode_differential(self, r, g, b):
        ...
        """
        diff = int(b) - int(r)
        # Normalize: (Diff / 186.15) * 255
        val = (diff / 186.15) * 255.0
    ...
    def scan(self):
        ...
        try:
            # Decode
            # Index 1: Spec Hash
            self.state['spec_hash'] = int(round(self.decode_differential(pixels[1][2], pixels[1][1], pixels[1][0])))
...
def main():
    ...
    while True:
        ...
        sys.stdout.write(f"\r[{combat_status}] Hash:{spec_hash} | HP:{data.get('player_hp',0):.1f}% | Res:{data.get('resource',0):.1f}% | Sec:{int(data.get('secondary',0))} | Snap:{data.get('snapshot',False)}   ")
        sys.stdout.flush()
        time.sleep(0.016)
