# SKILLWEAVER VISION ENGINE v25.12 (CALIBRATED)
# PROTOCOL: DARK MATTER (HIGH GAIN 0.73)
...
class VisionSystem:
    ...
    def decode_differential(self, r, g, b):
        """
        LUA: Base(0.08) + (Val/255 * 0.73)
        Max Diff = 0.73 * 255 = 186.15
        
        CALIBRATION:
        Observed signal loss ~5% (Pilot 78 vs 82, Hash 41.1 vs 43).
        To compensate, we lower the expected max diff.
        New Divisor: 186.15 * 0.95 = 177.0
        """
        diff = int(b) - int(r)
        # Normalize: (Diff / 177.0) * 255
        val = (diff / 177.0) * 255.0
        return max(0, min(255, val))

    def calibrate(self):
        print("------------------------------------------------")
        print("[INIT] DARK MATTER PROTOCOL ENGAGED.")
        print("[SCAN] Searching for Stealth Signal (BACKGROUND)...")
        print("------------------------------------------------")
...
def main():
    ...
    while True:
        ...
        # Display hash as float for debugging (as requested)
        sys.stdout.write(f"\r[{combat_status}] Hash:{spec_hash:.2f} | HP:{data.get('player_hp',0):.1f}% | Res:{data.get('resource',0):.1f}% | Sec:{int(data.get('secondary',0))} | Snap:{data.get('snapshot',False)}   ")
        sys.stdout.flush()
        time.sleep(0.016)
